...
func postLivecommentHandler(c echo.Context) error {
	...
	cachedLivestream := livestreamMap.Get(int64(livestreamID))
	if cachedLivestream != nil {
...
func fillLivecommentResponse(ctx context.Context, tx *sqlx.Tx, livecommentModel LivecommentModel) (Livecomment, error) {
	...
	cachedUser := userMap.Get(livecommentModel.UserID)
	if cachedUser != nil {
		commentOwnerModel = *cachedUser
	} else {
	...
	cachedLivestream := livestreamMap.Get(livecommentModel.LivestreamID)
	if cachedLivestream != nil {
		livestreamModel = *cachedLivestream
	} else {
...
func fillLivecommentReportResponse(ctx context.Context, tx *sqlx.Tx, reportModel LivecommentReportModel) (LivecommentReport, error) {
	...
	cachedReporter := userMap.Get(reportModel.UserID)
	if cachedReporter != nil {
		reporterModel = *cachedReporter
	} else {
